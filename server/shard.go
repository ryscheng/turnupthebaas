...
func NewShard(name string, socket string, globalConfig common.GlobalConfig) *Shard {
	...
	s.Table = NewTable(s.PirServer, name+"-Table", s.log.Info, globalConfig.BucketDepth, globalConfig.MaxLoadFactor, globalConfig.LoadFactorStep)
	...
	for {
		...
		case writeReq = <-s.writeChan:
			if err := s.processWrite(writeReq); err != nil {
				s.log.Error.Println("processWrite failed, ending processRequests loop")
				break
			}
			continue
		case batchReadReq = <-s.readChan:
			...
			continue
		case <-s.signalChan:
			s.log.Info.Printf("Shard Closing.")
			break
		}
	}
}
...
func (s *Shard) processWrite(req *common.WriteArgs) error {
	s.log.Trace.Printf("processWrite: enter with seqNo=%v\n", req.GlobalSeqNo)

	err := s.Table.Write(asCuckooItem(req))
	if err != nil {
		s.log.Error.Fatalf("Could not write item: %v", err)
		return err
	}
	s.sinceFlip += 1
	...
	if s.sinceFlip > maxWriteBuffer {
		...
		s.sinceFlip = 0
		if err != nil {
			s.log.Error.Fatalf("Could not flip write snapshot: %v", err)
			return err
		}
	}
	s.log.Trace.Printf("processWrite: exit\n")
	return nil
}
