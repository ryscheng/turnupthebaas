...
func TestFragment(t *testing.T) {
	...
	validateMsg := make([]byte, 1024*256)

	hdr := msgHeader{}
	hdr.left = uint32(len(theMsg))

	parts := make([][]byte, 0)
	for hdr.left > 0 {
		alloc := make([]byte, 256)
		adv := hdr.Fill(alloc, theMsg)
		theMsg = theMsg[adv:]
		parts = append(parts, alloc)
	}

	if len(parts) < 1024 {
		t.Fatalf("failed to fragment message")
	}

	// reconstruct
	partial := make([]byte, 0)
	incomingHdr := msgHeader{}

	done := false
	var err error
	for i := 0; i < len(parts); i++ {
		done, err = incomingHdr.Unpack(&partial, parts[i])
		if err != nil {
			t.Fatalf("unpack failed: %v", err)
		}
	}
	if !done {
		t.Fatalf("unpacking didn't indicate end of msg")
	}
	if len(partial) != len(validateMsg) {
		t.Fatalf("Failed to reconstruct fragmented msg (%d of %d bytes)", len(partial), len(validateMsg))
	}
}
